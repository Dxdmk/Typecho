    ...
    public static function install()
    {
        ...
        if (strpos($adapterName, 'Mysql') !== false) {
            ...
            try {
                ...
                # 初始化数据库如果不存在
                if (!$db->fetchRow($db->query("SHOW TABLES LIKE '{$prefix}ServerStatus_server';", Typecho_Db::READ)) && !$db->fetchRow($db->query("SHOW TABLES LIKE '{$prefix}ServerStatus_system';", Typecho_Db::READ))) {
